...
class Node(Node):
    ...
    def compute_accumulated_cost(self) :
        if (self.parent == None) :
            ...
            dist_from_parent_to_current_node = calculate_distance(points[real_parent.sensor],points[self.sensor])
    ...
    while True :
        ...
        if (is_HI_Mode):#Mode HI =>va que vers les capteurs HI du segment 
            if(points[node.sensor]["c"] == True):
                ...
                #Node LO deleted (les accumulated cost n'augmentent pas vu qu'on ne se déplace pas)
