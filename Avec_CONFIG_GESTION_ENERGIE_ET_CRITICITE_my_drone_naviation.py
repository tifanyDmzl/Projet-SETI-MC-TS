...
class DroneEnvironmentState(Node):
    def __init__(environment, total_dist, terminal, visited_sensors, energy_remaining):
    ...
    def find_children(environment):
        ...
        return {
            environment.choose_next_sensor(int(sensor[1:])) for sensor in unvisited_sensors
        }
    ...
    def find_closer_child(environment):
        ...
        return environment.choose_next_sensor(int(closest_sensor[1:]))#enlever le -1 /+1 ?
    ...
    def choose_next_sensor(environment, index):
        ...
        distance_to_next = calculate_distance(points[last_sensor], points[f"C{index}"])
        distance_back_to_base = calculate_distance(points[f"C{index}"], points["B"])
        ...
        if not terminal : 
            visited_sensors.append(f"C{index}")
            terminal = (len(visited_sensors)==len(points)-1) or energy_remaining < 0
        else : 
            energy_remaining = environment.energy_remaining #Décrémente pas l'énergie car si on est terminal on s'arrête là ! 
      

        return DroneEnvironmentState(total_dist=total_dist, terminal=terminal, visited_sensors=visited_sensors, energy_remaining = energy_remaining)
...
def do_drone_navigation(points, init_energy): 
    ...
    tree = MCTS()
    print("tree : ",tree)
    environment = new_drone_environment(init_energy)
    print (environment)
...
def new_drone_environment(init_energy):
    return DroneEnvironmentState(total_dist=0, terminal=False, visited_sensors=[], energy_remaining = init_energy)
